import base64, os, smtplib, subprocess
...
def checkConfig(sender, receiver, password, smtpsrv, port, tls):	
	...
	dir_path = script_path.replace("Rpi-Healthcheck.py","")
...
def sendMail(sender, password, smtpsrv, port, tls, receivers):
	script_path = os.path.abspath(__file__)
	dir_path = script_path.replace("Rpi-Healthcheck.py","")
	services_up = dir_path+"service_up"
	services_down = dir_path+"service_down"
	temperature = dir_path+"temperature"
	...
	try:
		TEMPERATURE = os.popen('/opt/vc/bin/vcgencmd measure_temp > {}'.format(temperature))
		TEMPERATURE = (open(temperature,"r").read().split("="))[1].replace("\n","")
	...
	running_services = os.popen("systemctl list-units --type=service | grep running | wc -l").read().replace("\n","")
	inactives_services = os.popen("""systemctl list-units --state=inactive | grep .service | sed 's/\|/ /' | awk '{print $1}' | wc -l""").read().replace("\n","")
	failed_services = os.popen("""systemctl list-units --state=failed | grep .service | sed 's/\|/ /' | awk '{print $1}' | wc -l""").read().replace("\n","")
	RUN_SRVC = running_services
	STP_SRVC = failed_services
	INAC_SRVC = inactives_services
	if int(STP_SRVC) >= 2:
	...
	if int(INAC_SRVC) >= 2:
		INAC_SRVC = """<strong style="color: #ff3300">{}</strong>""".format(INAC_SRVC)
	elif int(INAC_SRVC) >= 1:
		INAC_SRVC = """<strong style="color: #E9703E">{}</strong>""".format(INAC_SRVC)
	else: 
		INAC_SRVC = """<strong style="color: #00cc00">{}</strong>""".format(INAC_SRVC)
	ALL_SRVC = """<br><br><strong style="color: #ff3300"> FAILED  </strong><br>"""
	for service in (os.popen("""systemctl list-units --type service --all | awk 'BEGIN{};$4 ~ /^failed$/{print $2}' | column -t""").read()).split("\n"):
		ALL_SRVC += service
		ALL_SRVC += "<br>"
	ALL_SRVC += """<strong style="color: #D9B801">INACTIVE</strong><br>"""
	for service in (os.popen("""systemctl list-units --type service --all | awk 'BEGIN{};$4 ~ /^inactive$/{print $2}' | column -t""").read()).split("\n"):
                ALL_SRVC += service
                ALL_SRVC += "<br>"
	ALL_SRVC += """<strong style="color: #00cc00">RUNNING  </strong><br>"""
	for service in (os.popen("""systemctl list-units --type service --all | awk 'BEGIN{};$4 ~ /^running$/{print $1}' | column -t""").read()).split("\n"):
		ALL_SRVC += service
		ALL_SRVC += "<br>"
	ALL_SRVC = ALL_SRVC.replace(".service","")
	UPDATE_LIST = "<br>"
	available_update = (os.popen("""apt list --upgradeable 2> /dev/null | sed -n '1!p'""").read()).split("\n")
	if "" in available_update:
		UPDATE_LIST = "None <br>"
	for package in available_update:
		package = package.split("/")[0]
		UPDATE_LIST += package
		UPDATE_LIST += "<br>"
	NB_UPDATES = os.popen("""apt list --upgradeable 2> /dev/null | sed -n '1!p' | wc -l""").read().replace("\n","")
	if int(NB_UPDATES) >= 5:
	...
	with open(dir_path+'templates/report.html', 'r') as template:
		...
		html_code = html_code.replace("$INAC_SRVC",INAC_SRVC)
	...
	for receiver in receivers:
		...
		try:
			...
			msg['Subject'] = 'Rpi-Healthcheck report'
			msg['From'] = sender
			msg['To'] = receiver
